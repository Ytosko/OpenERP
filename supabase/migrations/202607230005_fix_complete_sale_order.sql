-- Migration: 202607230005_fix_complete_sale_order.sql
-- Fix: complete_sale inserted invoice_items BEFORE the parent invoices row,
-- violating invoice_items_invoice_id_fkey on every sale. The invoice header
-- is now inserted first (with zero totals) and totals are updated at the end.
...
BEGIN
  ...
  -- 8. Finalize Totals on the Invoice Header
  v_grand_total := ROUND(v_subtotal - COALESCE(p_discount_total, 0), 2);
  ...
  INSERT INTO public.audit_logs (
    ...
    jsonb_build_object('invoice_number', v_invoice_number, 'total', v_grand_total)
  );
